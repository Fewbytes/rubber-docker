...
# Level 10 - exec non-root containers
#
# Goal: allow specifying uid and gid for the container to run as
#
...
def _setup_memory_cgroup(container_id, memory, memory_swap):
    ...
    container_mem_cgroup_dir = os.path.join(MEMORY_CGROUP_BASEDIR, 'rubber_docker', container_id)
    ...
    if memory is not None:
        mem_limit_in_bytes_file = os.path.join(container_mem_cgroup_dir, 'memory.limit_in_bytes')
        open(mem_limit_in_bytes_file, 'w').write(str(memory))
    if memory_swap is not None:
        memsw_limit_in_bytes_file = os.path.join(container_mem_cgroup_dir, 'memory.memsw.limit_in_bytes')
